...
const BackgroundStack = styled(Stack)(({ theme }) => ({
  ...
  height: '95vh', // Slightly shorter to allow about section to peek through
  position: 'relative',
  overflow: 'visible', // Allow content to overflow for continuity
...
function Hero() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (heroContent) {
        heroContent.style.opacity = Math.max(1 - scrollPosition / 500, 0);
      ...
      <BackgroundStack direction="row" spacing={1} alignItems="center">
        ...
        >
          <Typography variant="h3" className="hero-text" gutterBottom>
            Hey there, <br /><span className="hero-name">Zephan Philip</span>
          </Typography>
          
          <Typography variant="h4" className="hero-text" paragraph>
            I'm a{' '}
            <ReactTyped
              strings={[ 'Jr MERN Stack Dev','Jr Unity Dev','3D Animator']}
              typeSpeed={150}
              loop
              backSpeed={20}
              cursorChar=">"
